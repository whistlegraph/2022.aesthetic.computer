...
const { widget } = figma;
const { useEffect, usePropertyMenu, useSyncedState, useWidgetId, AutoLayout, Text, } = widget;
function Widget() {
    ...
    usePropertyMenu([
        ...
        if (propertyName === "disks")
            setDisk(propertyValue);
    });
    return (figma.widget.h(AutoLayout, { verticalAlignItems: "center", spacing: 16, padding: 16, cornerRadius: 8, fill: { r: 0.8, g: 0.8, b: 0.9, a: 1 }, stroke: { r: 0.7, g: 0.7, b: 0.8, a: 1.0 }, strokeWidth: 6 },
        figma.widget.h(Text, { fontSize: 32, fill: { r: 0, g: 0.7, b: 0, a: 1 }, onClick: () => new Promise((resolve) => {
                ...
                allConnectorNodes.forEach((cn) => __awaiter(this, void 0, void 0, function* () {
                    // TODO: If end or start is widgetNode.id
                    //       and the type is an image... then process it as
                    //       an input (only use the first image).
                    console.log("Start:", figma.getNodeById(cn.connectorStart.endpointNodeId));
