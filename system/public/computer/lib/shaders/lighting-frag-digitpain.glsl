...
vec4 getColor(vec3 pos)
{
    ...
    if (abs(pos.z) < inputRadius && inBounds(imgCoords))
    {
        ...
        if (randomDirection.z > 0.8)
        {
          outColor.w = (1.0-maxv(outColor.xyz)) * innerDensity;
        }
    }
    return outColor;
}
...
void main()
{
    ...
    vec2 tm = -cameraDistance * 2. * (randomDirection.xy - vec2(.5));
    vec3 ro = vec3(tm, cameraDistance);
    vec3 rd = normalize(vec3(uv + tm / cameraDistance, focalLength));
